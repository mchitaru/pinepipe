...
<div class="scrollable-list col-lg-4 col-md-12">
    <div class="card-list">
        <div class="card-list-head">
        <h5>{{__($title)}} <span class="badge badge-{{$count ? 'warning':'light bg-white'}}">{{$count}}</span></h5>
        </div>
        <div class="card-list-body collapse show" id="{{$title}}" style="max-height:50vh">
            {{-- @if($ready) --}}
                ...
                <div class="card card-task">
                    ...
                    <div class="card-body">
                        <div class="card-title">
                            <a href="{{ route('tasks.show', $task->id) }}" data-remote="true" data-type="text" title="{{$task->title}}">
                                <h6 data-filter-by="text">{{$task->title}}</h6>
                            </a>
                            <span>{!!\Helpers::showDateForHumans($task->due_date)!!}</span>                            
                        </div>
