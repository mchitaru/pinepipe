...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        if(\Auth::user()->type == 'company'){
            ...
            return view('dashboard.company', compact('lead', 'project', 'invoice', 'top_tasks', 'top_due_invoice', 'users', 'projectData', 'taskData', 'last_project_stage', 'activities'));
            ...
            else
            {
                ...
                // $activities = Activity::where('created_by', \Auth::user()->creatorId())->limit(20)->orderBy('id', 'desc')->get();
                $activities = Activity::select('activities.*', 'user_projects.id as up_id')
                                        ->join('user_projects', 'user_projects.project_id', '=', 'activities.actionable_id')
                                        ->where('activities.actionable_type', '=', Project::class)
                                        ->where('user_projects.user_id', '=', \Auth::user()->id)->limit(20)->orderBy('id', 'desc')->get();
            }
